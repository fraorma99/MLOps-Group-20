...
The primary objective of this project is to develop and deploy a robust, scalable, and fully automated MLOps pipeline for Language Detection. In an increasingly globalized digital world, identifying the language of a given text is a fundamental preprocessing step for many NLP tasks. Our goal is not just to build a high-accuracy classifier but to implement a complete lifecycle management system—from data versioning and model training to containerized deployment and monitoring—following the best practices taught in the DTU MLOps course. We aim to provide an API where users can submit text fragments and receive the predicted language with high confidence.
