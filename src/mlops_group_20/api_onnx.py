...
app = FastAPI(title="Language Detection API - ONNX")
...
def load_artifacts():
    """Load ONNX model and metadata on startup."""
    global onnx_session, vocab, idx2label
    try:
        ...
        print(f"✓ Model Inputs: {[inp.name for inp in onnx_session.get_inputs()]}")
        print(f"✓ Model Outputs: {[out.name for out in onnx_session.get_outputs()]}")
...
def predict(request: TextRequest):
    ...
    # Inference with ONNX
    outputs = onnx_session.run(None, {"input_ids": input_array})
    logits = outputs[0][0]  # Shape: (num_classes,)
    print

    # Get prediction
    predicted_idx = np.argmax(logits)
    prediction = idx2label[int(predicted_idx)]
    return {
